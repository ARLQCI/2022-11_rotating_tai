...
# #  OCT for `t_r=100μs`, `V0=2.1MHz`

# We are optimizing for point (6) at 2.1 MHz in `2023-02-01_map_analysis.ipynb`. We are varying *only* ω(t), not V₀(t)
...
problem = ControlProblem(;
    ...
    verbose=true,
    pulse_options=IdDict(δω => Dict(:lambda_a => 1e7, :update_shape => t -> 1.0)),
...
res = optimize(problem; method=:krotov, iter_stop=50)
