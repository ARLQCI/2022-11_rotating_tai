...
function psi_ho_groundstate(θ; θ₀=0.125π, V₀=POTENTIAL_DEPTH, M=EFFECTIVE_MASS, m=N_SITES)
    ω = sqrt(V₀ * m^2 / M)
    a = 1 / sqrt(M * ω)
    c = 1 / (π^(1/4) * sqrt(a))
    Δθ = θ .- θ₀
...
# We see very close agreement between the haromic and the actual ground state
