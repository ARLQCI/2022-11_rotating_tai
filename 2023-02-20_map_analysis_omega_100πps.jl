...
plot!(; xlabel="momentum", ylabel="amplitude", xlim=(-1500, 1500))
vline!([-EFFECTIVE_MASS * OMEGA_TARGET], color="black", label=raw"$Mω_0$")

EFFECTIVE_MASS * OMEGA_TARGET
...
separation_time_orders_of_magnitude = collect(range(-1, 5, length=481))
...
F = run_or_load("./data/2023-02-20_map_splitting_fidelity_ω₀_100πps.npz"; force=false) do
...
heatmap(
    ...
    title=raw"Separation Fidelity $|⟨Ψ(t_r) | Ψ_{\textrm{tgt}}⟩|^2$ for ω₀₌100π/sec",
)

function scan_fidelity(V0, separation_time_values; kwargs...)
    M = length(separation_time_values)
    F = zeros(M)
    progress = Progress(M)
    Threads.@threads for j = 1:M
        t_r = separation_time_values[j]
        F[j] = propagate_splitting(t_r, V0; kwargs...)
        next!(progress)
    end
    return F
end

t_r_100kHz = [10^x * sec for x in range(-4, -1, length=500)]
fid_100kHz = scan_fidelity(0.1MHz, t_r_100kHz);

plot(
    t_r_100kHz./sec, fid_100kHz;
    xaxis=:log10, xticks=[1e-4, 1e-3, 1e-2, 1e-1], xminorticks=9,
    xlabel="sepatation time (seconds)",
    ylabel="separation fidelity",
    label="", title=raw"$t_r$ = 100kHz"
)

t_r_2MHz = [10^x * sec for x in range(-4, -1, length=500)]
fid_2MHz = scan_fidelity(2MHz, t_r_2MHz);

plot(
    t_r_2MHz./sec, fid_2MHz;
    xticks=[1e-4, 1e-3, 1e-2, 1e-1], xminorticks=9, xaxis=:log10,
    xlabel="sepatation time (seconds)",
    ylabel="separation fidelity",
    label="", title=raw"$t_r$ = 2MHz"
)
